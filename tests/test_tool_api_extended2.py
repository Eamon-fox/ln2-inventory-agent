...
class TestCellLineQueryDirect(unittest.TestCase):
    """Direct tool_query_inventory tests with cell_line as structural field."""

    def test_query_by_cell_line_structural(self):
        """Query with cell_line= should match top-level cell_line field."""
        with tempfile.TemporaryDirectory() as td:
            yaml_path = Path(td) / "inventory.yaml"
            write_yaml(
                make_data([
            ...
            self.assertTrue(result["ok"])
            self.assertEqual(2, result["result"]["count"])
            ids = {r["id"] for r in result["result"]["records"]}
            self.assertEqual({1, 3}, ids)

    def test_query_cell_line_combined_with_box(self):
        """cell_line + box filter should intersect correctly."""
        with tempfile.TemporaryDirectory() as td:
            yaml_path = Path(td) / "inventory.yaml"
            write_yaml(
                make_data([
            ...
            result = tool_query_inventory(str(yaml_path), cell_line="K562", box=2)
            self.assertTrue(result["ok"])
            self.assertEqual(1, result["result"]["count"])
            self.assertEqual(2, result["result"]["records"][0]["id"])

    def test_query_cell_line_no_match(self):
        """Query with non-existent cell_line should return empty."""
        with tempfile.TemporaryDirectory() as td:
            yaml_path = Path(td) / "inventory.yaml"
            write_yaml(
                make_data([
            ...
            result = tool_query_inventory(str(yaml_path), cell_line="MCF7")
            self.assertTrue(result["ok"])
            self.assertEqual(0, result["result"]["count"])

    def test_search_includes_cell_line(self):
        """tool_search_records should find records by cell_line value."""
        with tempfile.TemporaryDirectory() as td:
            yaml_path = Path(td) / "inventory.yaml"
            write_yaml(
                make_data([
            ...
            result = tool_search_records(str(yaml_path), query="HEK293T")
            self.assertTrue(result["ok"])
            self.assertEqual(1, result["result"]["total_count"])
            self.assertEqual(1, result["result"]["records"][0]["id"])


class TestCellLineAddEntryEndToEnd(unittest.TestCase):
    """End-to-end: add entry with cell_line, then query by it."""

    def test_add_then_query_by_cell_line(self):
        with tempfile.TemporaryDirectory() as td:
            yaml_path = Path(td) / "inventory.yaml"
            write_yaml(
                make_data([make_record(1, box=1, position=1)]),
            ...
            # Add with cell_line
            add_result = tool_add_entry(
                yaml_path=str(yaml_path),
                box=2,
                positions=[1],
                frozen_at="2025-01-15",
                fields={"cell_line": "NCCIT", "short_name": "e2e-test"},
                source="test_e2e",
            )
            self.assertTrue(add_result["ok"])

            # Query by cell_line
            query_result = tool_query_inventory(str(yaml_path), cell_line="NCCIT")
            self.assertTrue(query_result["ok"])
            # Should find the newly added record
            found = [r for r in query_result["result"]["records"] if r.get("short_name") == "e2e-test"]
            self.assertEqual(1, len(found))
            self.assertEqual("NCCIT", found[0]["cell_line"])
            self.assertEqual(2, found[0]["box"])

    def test_add_then_edit_cell_line_then_query(self):
        with tempfile.TemporaryDirectory() as td:
            yaml_path = Path(td) / "inventory.yaml"
            write_yaml(
                make_data([make_record(1, box=1, position=1)]),
            ...
            # Add with cell_line=K562
            add_result = tool_add_entry(
                yaml_path=str(yaml_path),
                box=2,
                positions=[1],
                frozen_at="2025-01-15",
                fields={"cell_line": "K562", "short_name": "edit-test"},
                source="test_e2e",
            )
            self.assertTrue(add_result["ok"])
            new_id = add_result["result"]["new_id"]

            # Edit cell_line to HeLa
            edit_result = tool_edit_entry(
                yaml_path=str(yaml_path),
                record_id=new_id,
            ...
            # Query by HeLa should find it
            query_result = tool_query_inventory(str(yaml_path), cell_line="HeLa")
            self.assertTrue(query_result["ok"])
            self.assertEqual(1, query_result["result"]["count"])
            self.assertEqual(new_id, query_result["result"]["records"][0]["id"])

            # Query by K562 should NOT find it
            query_k562 = tool_query_inventory(str(yaml_path), cell_line="K562")
            self.assertTrue(query_k562["ok"])
            self.assertEqual(0, query_k562["result"]["count"])
