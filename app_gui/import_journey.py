    ...
    def _build_ai_prompt(self, staged_inputs: List[str], preconvert: XlsxPreconvertResult) -> str:
        ...
        lines = [
            ...
            *normalized_assets,
            ...
            "4) Follow the returned skill instructions and read any referenced skill documents you need.",
            ...
            "7) Read and maintain migrate/output/migration_checklist.md as live progress (check items as completed).",
            "8) Run precheck and propose field mapping + schema plan for migrate/output/expected_schema.json, including optional box_tags mapping when source labels exist.",
            "9) Ask user to approve mapping via question, include options `APPROVE_MAPPING` and `REVISE_MAPPING`.",
            "10) Only after user selects `APPROVE_MAPPING`, lock expected_schema and convert source materials into migrate/output/ln2_inventory.yaml.",
            "11) Run `validate` with path `migrate/output/ln2_inventory.yaml`; if any errors or warnings remain, repair and re-run validation.",
            "12) For migration readiness, treat both `error_count > 0` and `warning_count > 0` as blockers, then update checklist status before import confirmation.",
            "13) Ask user for target dataset name via question, then keep that exact answer for `target_dataset_name`.",
            "14) Before importing, call question and include option `CONFIRM_IMPORT` for explicit user approval.",
            "15) Only after user selects CONFIRM_IMPORT, call import_migration_output with confirmation_token=CONFIRM_IMPORT and target_dataset_name=<from step 13>.",
            "16) Report final imported dataset path after the import tool succeeds.",
        ]
        return "\n".join(lines)
