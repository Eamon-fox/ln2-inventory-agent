    ...
    def _build_ai_prompt(self, staged_inputs: List[str], preconvert: XlsxPreconvertResult) -> str:
        ...
        lines = [
            ...
            *normalized_assets,
            ...
            "3) Use repo-relative paths consistently across file tools and shell commands; when you mean the migration workspace, spell paths as `migrate/...`.",
            "4) Follow the returned skill instructions. Prefer inline `reference_documents` / `shared_reference_documents` returned by `use_skill`; only call `fs_read` for additional repo-relative files you still need.",
            "5) If a conversion report is available, call fs_read on it and prioritize normalized CSV/schema assets.",
            "6) If pre-conversion is failed/partial or required sheets are missing, fall back to staged raw files.",
            "7) If migrate/output/expected_schema.json, migrate/output/migration_checklist.md, or migrate/output/ln2_inventory.yaml already exist, inspect them first and resume from the highest valid completed stage instead of replaying the workflow from scratch.",
            "8) Read and maintain migrate/output/migration_checklist.md as live progress (check items as completed).",
            "9) Run precheck and propose field mapping + schema plan for migrate/output/expected_schema.json, including optional box_tags mapping when source labels exist.",
            "10) Ask user to approve mapping via question, include options `APPROVE_MAPPING` and `REVISE_MAPPING`.",
            "11) Only after user selects `APPROVE_MAPPING`, lock expected_schema and materialize source materials into migrate/output/ln2_inventory.yaml.",
            "12) If the approved mapping is identity and the source file is already a valid LN2 YAML, use `fs_copy` instead of re-emitting the whole file through `fs_write`.",
            "13) Run `validate` with path `migrate/output/ln2_inventory.yaml`; if any errors or warnings remain, repair and re-run validation.",
            "14) For migration readiness, treat both `error_count > 0` and `warning_count > 0` as blockers, then update checklist status before import confirmation.",
            "15) Ask user for target dataset name via question, then keep that exact answer for `target_dataset_name`.",
            "16) Before importing, call question and include option `CONFIRM_IMPORT` for explicit user approval.",
            "17) Only after user selects CONFIRM_IMPORT, call import_migration_output with confirmation_token=CONFIRM_IMPORT and target_dataset_name=<from step 15>.",
            "18) Report final imported dataset path after the import tool succeeds.",
        ]
        return "\n".join(lines)
