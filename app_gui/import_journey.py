    ...
    def _build_ai_prompt(self, staged_inputs: List[str], preconvert: XlsxPreconvertResult) -> str:
        ...
        lines = [
            ...
            *normalized_assets,
            ...
            "",
            "Execution protocol:",
            "1) First call fs_read to load migration_assets/templates/prompt_en.md.",
            "2) Then call fs_read to load migration_assets/schema/validation_rules.md.",
            "3) If a conversion report is available, call fs_read on it and prioritize normalized CSV/schema assets.",
            "4) If pre-conversion is failed/partial or required sheets are missing, fall back to staged raw files.",
            "5) Read and maintain migrate/output/migration_checklist.md as live progress (check items as completed).",
            "6) Run precheck and propose field mapping + schema plan for migrate/output/expected_schema.json, including optional box_tags mapping when source labels exist.",
            "7) Ask user to approve mapping via question, include options `APPROVE_MAPPING` and `REVISE_MAPPING`.",
            "8) Only after user selects `APPROVE_MAPPING`, lock expected_schema and convert source materials into migrate/output/ln2_inventory.yaml.",
            "9) Use validate_migration_output only (do not run migration_assets/validate.py) to verify strict import readiness; if failed, repair and re-run validation.",
            "10) Confirm migrate/output/validation_report.json is refreshed by validate_migration_output, then update checklist status before import confirmation.",
            "11) Ask user for target dataset name via question, then keep that exact answer for `target_dataset_name`.",
            "12) Before importing, call question and include option `CONFIRM_IMPORT` for explicit user approval.",
            "13) Only after user selects CONFIRM_IMPORT, call import_migration_output with confirmation_token=CONFIRM_IMPORT and target_dataset_name=<from step 11>.",
            "14) Report final imported dataset path after the import tool succeeds.",
        ]
        return "\n".join(lines)
