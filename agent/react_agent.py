...
9) For environment/file/script tasks, use the smallest fitting file tool first: prefer `fs_list`/`fs_read` for inspection, `fs_edit` for targeted text replacement, and `fs_write` only for small text artifacts. When you need to materialize an existing whole file, prefer `fs_copy` over re-emitting the file content through `fs_write`. Use `bash` (bash -lc) or `powershell` (powershell -Command) for command/script execution, and always include a concise `description`. Consult the "Directory context" section for repo root, read/write scopes, and path format.
