...
def _build_batch_move_plan(
    *,
    ...
    for row_idx, entry in enumerate(normalized_entries, 1):
        if len(entry) < 3:
            errors.append(f"Row {row_idx}: move requires id:from->to format")
            continue
        ...
        if dest_idx == idx:
            errors.append(f"Row {row_idx} ID {record_id}: target position {to_pos} already belongs to this record")
            continue
        ...
        source_event = api._build_move_event(
            date_str=date_str,
