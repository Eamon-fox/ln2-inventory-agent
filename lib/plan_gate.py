...
This module lives in ``lib/`` so both ``agent/`` and ``app_gui/`` can import
it without creating a reverse dependency from agent → GUI.
"""
...
def validate_plan_batch(
    *,
    ...
    Args:
        preflight_fn: Optional callable ``(yaml_path, items, bridge) -> report``.
            When *None*, preflight is skipped even if *run_preflight* is True.
            GUI callers pass ``app_gui.plan_executor.preflight_plan`` here.
    """
    ...
    if run_preflight and preflight_fn and valid_items and yaml_path and os.path.isfile(yaml_path):
        try:
            preflight_report = preflight_fn(yaml_path, valid_items, bridge)
